...
public abstract class ReadElement {
	...
	public static MainChapter loadMch(File save) {
		return (MainChapter) loadSch(save.isDirectory() ? new File(save.getPath(), "main.json") : save, null, null);
	}
	...
	public static List<Data> readChildren(Source src, boolean name, boolean sf, boolean desc, Container parent, String... tags) {
		List<Data> data = new java.util.ArrayList<>();
		try {
			while (true) {
				dumpSpace(src, '{', ',', '\n', '\t');
	 ...
	 * {@code ignore} parameter using {@link #dumpSpace(com.schlmgr.IOSystem.ReadElement.Source, char, char...)
	 * } method.
	 *
	 * @param src    {@link Source}
	...
	/**
	 * Creates an {@link com.schlmgr.objects.templates.BasicData} from the loaded data.
	 *
	 * @param src {@link Source}
