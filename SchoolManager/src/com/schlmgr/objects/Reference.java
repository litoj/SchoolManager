...
public final class Reference implements BasicData {
	...
	private static Reference mkElement(Object ref, List<Container> path, Object[] refPath) {
		...
		if (ref instanceof Container && !(ref instanceof TwoSided)) for (Container c : path)
			if (c == ref)
		...
		return new Reference(ref, refPath, (MainChapter) path.get(0));
	}
	...
	private static BasicData find(String name, Container par, Container parpar, boolean end) {
		...
		new Thread(() ->
				defaultOFLs.get(Reference.class + ":not_found").onFail(null, name, par, parpar)).start();
		return null;
	}
	...
	public static BasicData readData(com.schlmgr.IOSystem.ReadElement.Source src, Container parent) {
		Data data = com.schlmgr.IOSystem.ReadElement.get(src, true, false, false, false, parent, "origin");
